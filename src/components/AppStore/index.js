...
import {MainCon,Head,SearchCon,SearchIcon,Input,TabsUlList,AppsUlList} from './styledComponents'
...
const AppStore=()=>{
      ...
      const searchResult=getList().filter(each=>each.appName.toLowerCase().includes(input.toLowerCase()))
    return(
        <MainCon>
            <Head>App Store</Head>
            <SearchCon>
                <Input type="search" placeholder="Search" onChange={changeInput} value={input}/>
                <SearchIcon src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png" alt="search icon" />
            </SearchCon>
            <TabsUlList>
                {tabsList.map(eachTab=>(
                    <TabItem tabDetails={eachTab} key={eachTab.tabId} clickTabs={clickTab} isActive={eachTab.tabId===activeTabId} />
                ))}
            </TabsUlList>
            <AppsUlList>
